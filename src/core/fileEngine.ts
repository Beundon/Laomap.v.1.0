...
/** OGC GeoPackage — stored as a raw blob; full vector extraction is a v2 enhancement. */
async function importGeoPackage(file: File): Promise<ImportResult> {
  return {
    ...
    rawBlob: file,
    warnings: [
      'GeoPackage stored locally as a binary asset (SQLite-based OGC format). ' +
        'Layer-level vector extraction requires the optional sql.js module.',
    ],
  };
}
